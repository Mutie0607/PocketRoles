...
1. **[GitHub Releases](https://github.com/wakayamachannel/PocketRoles/releases)** から 2 つの zip をダウンロードします: `PocketRoles-Setup-0.5.4.zip`（ランチャー）と `PocketRoles-0.5.4.zip`（MOD 本体）。
...
- ホスト支援ツール: ロビーの残り時間表示、自動開始、廃村（ロビー更新）、開始のキャンセルボタン、会議の強制終了、ホットキー（F7 / F8 / F9）、ホストページの操作ボタン、ゲームマスター（観戦）モード、逆スケルド（Dleks）、高 PING の部屋の自動作り直し（v0.5.4 から既定オン）
...
- **Aegis アンチチート**（v0.5.3〜）: 登録オフの部屋の試合中、ありえない操作（キルできない役職のキル、スピードハック、チャットの連投など）を見つけて退出させます。v0.5.4 からはランチャーと一緒に **Aegis のトレイアプリ**（スキャン画面、通知領域のアイコン、起動前の検査、GitHub から更新される定義ファイル）も動きます
...
- ホストの画面左上（ping 表示）に `PocketRoles v0.5.4 (host) · アジア` のように接続中の地域名と、オンラインのロビーでは `ロビー残り mm:ss` が表示され（`/code on` にすると大きな部屋コード `役職部屋 ABCDEF` も出ます。既定オフ）、ゲーム内の MOD スタンプも表示されます。タイトル画面では右側の大きな窓に **PocketRoles のパネル**（アイコン、`v0.5.4 / Among Us 2026.8.18`、作者名、クリックで開く GitHub の行、「ホストだけ導入で役職が遊べる」「参加者は何も入れずに遊べる」）が出ます（[第 9 章](#9-歯車メニューのpocketroles-設定パネル)）。
...
- **高 PING の作り直しは確認式**: 部屋を作った直後の PING が高い時、ホストの画面に「PING が ○ ms と高いです。部屋を作り直しますか？」のダイアログ（はい / いいえ、`/rehost yes|no` でも可）。既定はオフ（`MaxHostPing = 0`）。短時間に何度も作り直すと ban points が付くためです（[3.4](#34-ban-とキックについて)）。**v0.5.4 からは既定でオン（80 ms）で、15 秒答えがなければ自動で作り直します。**
...
6. タイトル画面の右側に PocketRoles のパネル、左上に `PocketRoles v0.5.4` が出れば導入完了です。
...
5. コピー先の `Among Us.exe` を起動します（Steam ライブラリからではなく、コピー先の exe を直接。Steam は起動しておく）。画面左上に `PocketRoles v0.5.4`、タイトル画面の右側の窓に PocketRoles のパネルが出れば MOD が読み込まれています。`BepInEx\LogOutput.log` に `PocketRoles v0.5.4 loaded` と出ていることでも確認できます。
...
| `v0.5.4 / Among Us 2026.8.18` | MOD のバージョンと対応するゲームバージョン |
...
- ゲームの UI が変わってパネルが作れなかった時は、v0.2 の右下のクレジット行（`PocketRoles v0.5.4  © 2026 もみじちゃ` と URL）が代わりに出ます（バニラのバージョン表示と重ならないように上へ寄せます）。
- 何も送信しません（ホストの画面だけ）。
...
| `ac`（`anticheat`、`aegis`） | Aegis アンチチート（v0.5.3、v0.5.4 で強化）の記録の一覧。`/ac clear` で消去、`/ac on\|off`、`/ac kick on\|off`、`/ac test <kill\|vent\|ability\|task\|chat\|sabotage\|killcd\|protect\|distance\|rpc\|taskburst\|report\|teleport\|killphase\|callout\|chatflood\|name\|color\|speed\|ventfar> <#番号\|名前> [kick]` で判定と通知を試す（`kick` を付けた時だけ本当に退出） |
...
| `lobby.maxping`（`maxping`） | 0〜300（0 = 無効） | `[Lobby] HostPingLimit`（v0.5.4、旧 `MaxHostPing`。部屋を作った直後の PING がこれを超えたら作り直しを確認し、15 秒答えがなければ作り直す。既定 80） |
...
HostPingLimit = 80              # v0.5.4（旧 MaxHostPing）部屋を作った直後 5 秒間 PING がこの値(ms)を超え、自分しかいなければ「作り直しますか？」と確認し、15 秒答えがなければ作り直す（0〜300、0 = しない、連続 3 回まで）
...
### 高 PING の部屋を作り直す（`[Lobby] HostPingLimit`、`/opt maxping <ms>`）

公式のアジア地域でも、部屋を作るたびに割り当てられるサーバーが変わり、PING が 8〜10 ms（東京）だったり 70 ms 台や 100 ms 以上（遠いサーバー。9/21 には 112〜179 ms、スマホでは 500 ms 超え）だったりします。v0.5.4 からは既定でオン（`HostPingLimit = 80`）で、部屋を作った直後の 5 秒間 PING が上限を超えていて、まだ自分しかいない時に、ホストの画面に **「PING が ○ ms と高いです（遠いサーバー）。部屋を作り直しますか？（誰も入っていない間だけ。15 秒答えがなければ作り直します）」** のダイアログ（はい / いいえ）が出ます。チャットの `/rehost yes` / `/rehost no` でも答えられます。
...
1. **タイトル画面のパネル**: PC のメインメニュー右側に PocketRoles のパネル（アイコン、`v0.5.4 / Among Us 2026.8.18`、GitHub の行）が出て、GitHub の行をクリックするとブラウザが開くこと。オンラインメニューを開くと隠れ、戻ると再表示されること。
...
- 動作確認: MOD 用コピーの `Among Us.exe` を起動し、`BepInEx\LogOutput.log` に `PocketRoles v0.5.4 loaded` が出ていること、Harmony のパッチエラーが無いことを確認します。
