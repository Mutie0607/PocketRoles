...
| `about`, `info`, `説明` | MOD の説明（v0.5.2）。登録オフの部屋では「この部屋(役職なし)でMODがしていること: 入室時の挨拶と案内, 外国語チャットの自動翻訳, 試合後の結果一覧(途中で抜けた人も含む), 部屋の時間切れ防止」と「ゲームの中身は普通のAmong Usで, 役職や特殊ルールはありません」の 2 行、登録ありの部屋では役職 MOD の 1 行案内 |
