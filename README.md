...
- 設定タブの「ホスト」ページの上にも **今すぐ開始 / キャンセル / 廃村 / 会議終了 / テストモード / 設定を表示 / 次の自分** のボタンがあります（マウスだけで操作できます）。
...
| `me [impostor \| crew \| auto \| <本体の役職名>]` | 次の 1 試合の自分の役を指定（v0.5.1）。テストモード不要、登録オフの部屋でも可。設定タブ「ホスト」の「次の自分」ボタンでも。例: `/me impostor`、`/me shapeshifter`。`/me` だけで現在の指定 |
...
- v0.5.1: 登録オフでも `/me impostor` / `/me crew` / `/me <本体の役職名>` で次の 1 試合の自分の役を決められます（設定タブ「ホスト」の「次の自分」ボタンでも）。本体の役職選択の中で役職メッセージの宛先を入れ替えるだけなので、参加者には普通の配役に見え、インポスターの人数も変わりません。
