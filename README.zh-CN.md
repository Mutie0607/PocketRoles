...
**这是一个只需建房的人安装的 Among Us（2026.8.18 / Steam）职业模组。** 朋友们用 PC、手机或 Switch 上平时的 Among Us，输入房间代码就能加入。警长、豺狼、小丑等 26 种职业会通过名字标签和聊天悄悄地只告诉本人。设置在房间的电脑里完成，提示支持 日本語 / 中文 / English 三种语言，外语聊天自动翻译，房间超时和拖沓的会议也能由房主一键处理。免费、非营利，源代码在 GitHub 公开（**v0.5.1**，旧名 HostRoles）。
...
- 设置标签页“房主”分页的上方也有 **立即开始 / 取消 / 废村 / 结束会议 / 测试模式 / 显示设置 / 下局的我 / 下局内鬼** 按钮（只用鼠标即可操作）。
...
- 聊天中可以用 `/vset <项目> <值>` 直接设置（仅在房间中）。项目：`killcd`（`kill` `cooldown`）、`vote`、`discuss`、`emergency`、`common` / `short` / `long`（任务数）、`speed`（移动速度 0.25〜5 倍）、`vision`（船员视野 0.1〜10 倍）、`impvision`（内鬼视野 0.1〜10 倍）。职业项目（v0.5.0）：`vitalscd` `vitals`（科学家）、`ventcd` `venttime`（工程师）、`shiftcd` `shift`（变形者）、`phantomcd` `phantom`、`gacd` `ga`（守护天使）、`trackcd` `trackdelay` `track`（追踪者）、`noise`、`viper`、`detective`、`judge`、`vitalscrew`、`ventuses`（例：`/vset vitals 60`、`/vset shiftcd 3`）。`/vset show`（或只输入 `/vset`）显示当前值。速度和视野的扩展是固定范围，只能通过 `/vset` 设置（不会放宽设置界面的行）。 未注册房间里，给 `common` / `short` / `long` 指定超出原版范围的值时，设置保持在范围内，只改变实际分发的数量（v0.5.1）。
...
| `about`, `info`, `説明` | MOD 说明（v0.5.2）。未注册房间显示两行：“本房间(无职业)里MOD做的事: 入房问候和指引, 外语聊天自动翻译, 赛后结果一览(包括中途退出的人), 防止房间超时”和“游戏本身就是普通的Among Us, 没有职业和特殊规则”；已注册房间显示一行职业 MOD 的说明 |
...
- v0.5.1：当原版职业分配选出的内鬼少于房间设置（内鬼职业设为 100% 时会发生）时，会把原版选入内鬼名额的人恢复为内鬼（仍不足时从普通船员中补足），并在房主聊天中显示“内鬼只有 3 名中的 2 名，已把 〇〇 补为内鬼。”已注册的房间同样适用。
