...
- 设置标签页“房主”分页的上方也有 **立即开始 / 取消 / 废村 / 结束会议 / 测试模式 / 显示设置 / 下局的我** 按钮（只用鼠标即可操作）。
...
| `me [impostor \| crew \| auto \| <原版职业名>]` | 指定下一局自己的职业（v0.5.1）。无需测试模式，未注册房间也可；设置页“房主”的“下局的我”按钮亦可。例：`/me impostor`、`/me shapeshifter`；只输入 `/me` 显示当前指定 |
...
- v0.5.1：未注册房间也可用 `/me impostor` / `/me crew` / `/me <原版职业名>` 指定下一局自己的职业（设置页“房主”的“下局的我”按钮亦可）。只是在原版职业分配过程中交换职业消息的收件人，玩家看到的是普通的分配，内鬼人数不变。
